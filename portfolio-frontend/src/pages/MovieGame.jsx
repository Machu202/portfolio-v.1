...
export default function MovieGame() {
  ...
  useEffect(() => {
    fetch(`http://localhost:8080/api/posts?category=MovieGame&page=${currentPage}&size=5`)
  ...
  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this post?")) {
      ...
      .then(() => {
        setEvents(events.filter(eventPost => eventPost.id !== id));
      })
      ...
      {loading ? <p>Loading...</p> : (
        <div className="card-list">
          {events.map((eventPost) => (
            <article className="event-post" key={eventPost.id}>
              {eventPost.imageUrl && (
                <img 
              ...
              <button 
                onClick={() => handleDelete(eventPost.id)}
